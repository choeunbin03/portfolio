    ...
    challenges: [
      {
        ...
        content: '회원가입 시 유효성 검사를 프론트와 백엔드 양쪽에서 모두 해야 하는지에 대한 의문이 들어 조사한 결과, 프론트엔드는 사용자 경험 개선을, 백엔드는 보안과 데이터 무결성을 담당한다는 것을 이해했습니다.'
      },
      {
        title: 'Token 재발급을 통한 사용자 경험 개선',
        content: 'Refresh Token으로 Access Token을 재발급받는 로직을 구현하면서, 사용자가 로그인 상태를 유지하면서도 보안을 강화할 수 있는 방법을 고민했습니다.'
      },
      {
        title: '라이브러리 선택의 중요성',
        content: 'react-date-range의 DateRangePicker를 사용하면서 편리함과 동시에 커스터마이징의 제약을 경험했습니다. 다음 프로젝트에서는 여러 라이브러리를 비교 분석 후 신중하게 선택하겠습니다.'
      },
      {
        title: '컴포넌트 설계와 책임 분리',
        content: '여행 계획 페이지의 컴포넌트 구조를 설계하면서 유지보수성과 재사용성을 고려한 컴포넌트 분리의 중요성을 깨달았습니다.'
      }
    ],
    reflection: {
      learned: [
